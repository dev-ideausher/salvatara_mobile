...
class AppInterceptors extends Interceptor {
  ...
  Future<dynamic> onError(DioError dioError, ErrorInterceptorHandler handler) async {
    ...
    try {
      ...
      showSnakbar == true ? snackBarClass("${errorMessage}").ErrorSnackBar : null;
    } catch (e) {
      print(e);
    }

    if (dioError.response!.statusCode == 500 || dioError.response!.data["success"] == false) {
      final completer = Completer<Response<dynamic>>();
      await refreshToken().then((_) async {
        completer.complete(await retry(dioError.requestOptions));
      }).catchError((_) {
  ...
  Future<Response<dynamic>> retry(RequestOptions requestOptions) async {
    final options = Options(
      ...
      headers: {
        ...
        "token": Get.find<GetStorageService>().encjwToken, // updated token
        "type": "app"
      },
    );
  ...
  Future<bool> refreshToken() async {
    try {
      ...
      if (newToken != null) {
        Get.find<GetStorageService>().encjwToken = newToken;
        return true;
      } else {
        return false;
      }
...
//       print(e);
//     }
//      if ((dioError.response?.statusCode == 500 || dioError.response?.data['message'] == "invalid token")) {
...
//   Future<bool> refreshToken() async {
//     try {
//       Get.find<GetStorageService>().encjwToken = (await FirebaseAuth.instance.currentUser?.getIdToken(true))!;
